...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6 md:mb-8">
    <div>
        ...
        <p class="text-secondary text-sm md:text-base">Create a new class for your courses</p>
    </div>
    <a href="{{ route('dosen.classes.index') }}" class="flex items-center gap-2 px-6 py-3 ring-1 ring-border hover:ring-primary rounded-full text-foreground font-semibold transition-all duration-300 cursor-pointer">
        <i data-lucide="arrow-left" class="w-5 h-5"></i>
        <span>Back</span>
    </a>
</div>

<form action="{{ route('dosen.classes.store') }}" method="POST">
    @csrf
    ...
    <div class="flex flex-col rounded-2xl border border-border bg-white overflow-hidden">
        <div class="p-6 md:p-8 border-b border-border">
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <p class="text-secondary">Enter class details</p>
                </div>
            </div>
                ...
                <div class="space-y-2">
                    ...
                    <select name="schedule_day" class="w-full px-4 py-3 rounded-2xl ring-1 ring-border focus:ring-2 focus:ring-primary outline-none transition-all duration-300">
                        ...
                        @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'] as $day)
                            <option value="{{ $day }}" {{ old('schedule_day') == $day ? 'selected' : '' }}>{{ $day }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
        ...
        <div class="p-6 md:p-8 flex justify-end gap-4">
            <a href="{{ route('dosen.classes.index') }}" class="px-8 py-3 ring-1 ring-border hover:ring-primary rounded-full text-foreground font-semibold transition-all duration-300">Cancel</a>
